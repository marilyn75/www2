                ...
                <div class="row col-lg-12 col-xl-8">
                    ...
                    <div class="col-12">
                        ...
                        <ul class="sale_lst">
                            ...
                            <li>
                                <p>토지</p>
                                <p>{{ number_format($data['면적']['토지'], 2) }}㎡ ({{ number_format(@$data['면적']['토지p'], 2) }}평)
                                </p>
                            </li>
                            <li>
                                <p>건물</p>
                                <p>{{ number_format($data['면적']['건물'], 2) }}㎡ ({{ number_format(@$data['면적']['건물p'], 2) }}평)
                                </p>
                            </li>
                            <li>
                                <p>감정</p>
                    ...
                    <div class="col-12">
                        ...
                        <ul class="auc_width_wrap">
                            <li>
                                ...
                                <h3 class="auc_wid_n">{{ number_format($data['면적']['토지'], 2) }}㎡
                                    ({{ number_format(@$data['면적']['토지p'], 2) }}평)</h3>
                            </li>
                            <li>
                                <p class="auc_wid_tit">건물</p>
                                <h3 class="auc_wid_n">{{ number_format($data['면적']['건물'], 2) }}㎡
                                    ({{ number_format(@$data['면적']['건물p'], 2) }}평)</h3>
                            </li>
                        </ul>
                        <table>
                            <thead>
                    ...
                    @if (count($data['물건세부정보']['면적정보']) == 1 &&
                            $data['물건세부정보']['면적정보'][0]['종별'] == '토지' &&
                            !empty($data['토지정보']))
                        <!-- 토지정보&이용계획 -->
                        <div class="col-12">
                            ...
                            <table>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="bck_wt">
                                            {{ number_format($data['토지정보']['공시지가']) }}원 ({{ $data['토지정보']['공시년월'] }})</td>
                                    </tr>
                                </tbody>
                            </table>
                            <div class="cont_und_line"></div>
                            ...
                            <tbody>
                                @foreach ($data['압류재산정보']['등기부현황'] as $_row)
                                    ...
                                    <tr>
                                        ...
                                        <td class="bck_wt">
                                            @if ($_row['설정금액'] > 0)
                                                {{ number_format($_row['설정금액']) }}원@else{{ __('-') }}
                                            @endif
                                        </td>
                                    </tr>
                                @endforeach
                    ...
                    <div class="col-12">
                        ...
                        <ul class="auc_use_state">
                            <li>
                                <div class="use_state_list">
                                    ...
                                    <p style="white-space:pre-line;">{{ empty($data['유의사항'][0]) ? '-' : $data['유의사항'][0] }}
                                    </p>
                                </div>
                            </li>
                            <li>
                                <div class="use_state_list">
                                    ...
                                    <p style="white-space:pre-line;">{{ empty($data['유의사항'][1]) ? '-' : $data['유의사항'][1] }}
                                    </p>
                                </div>
                            </li>
                            <li>
                        ...
                        <p class="sm_txt">* 임대차정보는 감정서상 표시내용 또는 신고된 임대차 내용으로서 누락, 추가, 변동 될 수 있으니 참고 자료로만 활용하여야 하며, 이에 따른 모든
                            책임은 입찰자에게 있습니다. 임차인의 배분요구 여부는 입찰시작 7일전부터 제공하는 공매재산명세서를 통하여 확인하시기 바랍니다.</p>
                        <div class="cont_und_line"></div>
                    </div>
                            ...
                            <tbody>
                                @if (count($data['압류재산정보']['배분요구채권신고현황']) > 0)
                                    @foreach ($data['압류재산정보']['배분요구채권신고현황'] as $_row)
                                        <tr>
                                            <td class="bck_wt" rowspan="2">
                                                {{ !empty($_row['설정일자']) ? $_row['설정일자'] : '-' }}</td>
                                        ...
                                        <tr>
                                            <td class="bck_wt">
                                                {{ $_row['설정금액'] > 0 ? number_format($_row['설정금액']) . '원' : '-' }}</td>
                                            <td class="bck_wt">
                                                {{ $_row['배분요구채권액'] > 0 ? number_format($_row['배분요구채권액']) . '원' : '-' }}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
